...
public final class Main {
	...
	static WebServer startServer() throws IOException {
		...
		final ServerConfiguration serverConfig = ServerConfiguration.builder(config.get("server"))
			.tracer(TracerBuilder.create("helidon-se").buildAndRegister())
			.build();
	...
	private static Routing createRouting(final Config config) {
		...
		final HealthSupport health = HealthSupport.builder()
			...
			.add(() -> HealthCheckResponse.named("custom")
				.up()
				.withData("timestamp", new Date(System.currentTimeMillis()).toString())
				.build())
			.build();
	...
	private static Config buildConfig() {
		final String filepath = "D:\\Development\\workspace\\microprofile-helidon\\conf\\dev-conference-se.yaml";
		return Config.builder()
			.sources(file(filepath).pollingStrategy(PollingStrategies::watch).optional(),
